# attack/augmp.py
# AugMP attacker: graph-representation-learning model-manipulation attack.
#
# Ported (verbatim, algorithm unchanged) from the AugMP research codebase
# (GuangLun2000/AugMP) into HMP-GNN's attack/ package as a STRONG, STEALTHY,
# LEARNED update-manipulation baseline -- complementary to the classical
# byzantine baselines (sign_flipping / gaussian / alie) and to the paper's
# Hallucination data-poisoning attack.
#
# Threat model (data-agnostic / omniscient):
#   - The attacker performs NO local training (local_train returns a zero
#     update); the malicious update is CONSTRUCTED in camouflage_update from
#     the benign clients' updates it receives each round (server Phase 3).
#   - It uses a VGAE to learn the benign-update graph structure, a GSP module
#     to synthesise a graph-conditioned malicious direction, then a Lagrangian
#     constrained proxy optimisation that MAXIMISES the global-loss proxy
#     F(w'_g) subject to distance + two-sided cosine-similarity constraints
#     (mimics benign updates -> evades geometric/robust-aggregation defenses).
#   - F(w'_g) is estimated on a small INDEPENDENT clean proxy set obtained via
#     data_manager.get_proxy_eval_loader() (held-out from train, disjoint from
#     the test set -- see data_loader.DataManager). Set attacker_use_proxy_data
#     = False for the weaker constraint-only (no-data-access) variant.
#
# Server integration: HMP-GNN's server Phase 3 already drives the AugMP
# protocol (receive_benign_updates -> receive_attacker_updates ->
# camouflage_update), so no server changes are needed. Whatever defense is
# configured in Phase 4 (fedavg / hmp_gae / krum / ... ) then scores these
# crafted updates.
#
# The class inherits HMP-GNN's client.Client base (identical API to AugMP's
# own Client base: get_flat_params / set_flat_params / get_model_update).
...
class AttackerClient(Client):
    ...
    def __init__(self, client_id: int, model: nn.Module, data_manager,
        ...
        # Marks an update-FORGING attacker: it does NO local training, so
        # client.model stays == w_global and the malice lives only in the crafted
        # Delta returned by camouflage_update. The server uses this flag to (a) run
        # its Phase 0.5 constraint setup and (b) evaluate the semantic trust signal
        # on w_global + Delta instead of client.model. Attackers that bake their
        # poison into client.model via local training (e.g. Hallucination) do NOT
        # set this, so their server-side code path is byte-for-byte unchanged.
        self.crafts_update = True
